...
struct AG { // convert orientation matrix A to metrical matrix g & reverse
  ...
  */
  ...
  void forward(cctbx::crystal_orientation const &ori) {
    ...
    B = mat3(std::cos(phi), 0., std::sin(phi), 0., 1., 0., -std::sin(phi), 0.,
             std::cos(phi));
    mat3 BA(B * A);

    psi = std::atan2(-BA[5], BA[8]);
    C = mat3(1., 0., 0., 0., std::cos(psi), std::sin(psi), 0., -std::sin(psi),
             std::cos(psi));
    mat3 CBA(C * BA);

    theta = std::atan2(-CBA[1], CBA[4]);
    D = mat3(std::cos(theta), std::sin(theta), 0., -std::sin(theta),
  ...
  void validate_and_setG(uc_sym_mat3 const &g) {
    ...
    // Note:g0 = a*.a*  g1 = b*.b*  g2 = c*.c*  g3 = a*.b*  g4 = a*.c*  g5 =
    // b*.c*

    if (g2 <= 0.) {
      throw scitbx::error("g2 <= 0.");
      g2 = 1.E-7;
    }
    ...
    if (g1 - bstrz * bstrz <= 0.) {
      throw scitbx::error("g1-bstrz*bstrz <= 0.");
      g1 = bstrz * bstrz + 1.E-7;
    }
    double bstry = std::sqrt(g1 - bstrz * bstrz);
    double astry = (g3 - astrz * bstrz) / bstry;
    if (g0 - astry * astry - astrz * astrz <= 0.) {
      throw scitbx::error("g0 - astry*astry -astrz*astrz <= 0.");
      g0 = astry * astry + astrz * astrz + 1.E-7;
    }
    ...
    if (ersatz_uc.volume() <= 20.) {
      throw SCITBX_ERROR("Metrical matrix g is expected to be in the "
                         "reciprocal setting;this appears to be direct space");
    }
  };
  ...
  cctbx::crystal_orientation back_as_orientation() const {
