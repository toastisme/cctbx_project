    ...
    for rot_sign in [1, -1]:
        ...
        if derivs is not None:
            ...
            for d_theta_d_eta in d_theta_d_etas:
                dU_d_eta = dU_d_theta*(rot_sign*d_theta_d_eta)
...
if __name__ == "__main__":
    ...
    U, Uprime, Udblprime =generate_Umats(etas, crystal=cryst, num_random_samples=Nmos, plot="random angle sampling along spiral" if plot else None, how=2, verbose=True, compute_derivs=True)
    from simtbx.nanoBragg.tst_gaussian_mosaicity import plotter2
    etas_2 = (a/2.,0,0, 0,a/2.,0,0,0,a/2.)
    U_by2, _,_ =generate_Umats(etas_2, crystal=cryst, num_random_samples=Nmos, plot="random angle sampling along spiral" if plot else None, how=2, verbose=True, compute_derivs=False)
    ...
    check_finite(U, Ushift_minus, Uprime, -eps)
    print("-Finite differences check!")
    ...
    generate_Umats(etas, cryst, num_random_samples=Nmos,
                   plot="3-parameter anisotropic random sampling along spiral" if plot else None, how=1, compute_derivs=False)

    # fully anisotropic cap model
    etas = (a, d, f,
            d, b, e,
            f, e, c)
    generate_Umats(etas, cryst, num_random_samples=Nmos, plot="6-parameter anisotropic sampling along spiral" if plot else None, how=0)
